...
abstract class EnemyCore {
  ...
  /// True while this enemy cannot deal contact damage (e.g. a mimic still
  /// disguised or mid-reveal). The session checks it in the contact loop.
  bool get harmless => false;

  /// Camera distance (px) beyond which this enemy sleeps. Most enemies
  /// use the shared 1.5-screen budget; late wakers override.
  double get wakeDistance => kEnemySleepDistance;

  /// Whether spikes/fire kill this enemy on contact. Only enemies that can
  /// actually walk into a hazard opt in — a ledge-dropper stuck wading in a
  /// lava trench is a bug, a ledge-dropper that burns is a lesson.
  bool get hazardsKill => false;
  ...
  bool overlapsBody(Body other) => overlaps(other.x, other.y, other.w, other.h);
}
...
class ThornlingCore extends EnemyCore {
  ...
  /// For subtypes that reuse the patrol/hunt brain under another identity
  /// (the bramble mimic's revealed form).
  ThornlingCore.asKind(
    EnemyKind kind, {
    required super.x,
    required super.y,
    super.hp = 6,
  }) : super(kind: kind, w: 24, h: 22);
...
/// Bramble Mimic: sits disguised as a bush until the player comes close or
/// pokes it, shivers for a telegraphed beat (harmless — that is the player's
/// window), then fights with the thornling patrol/hunt brain. Fair by
/// construction: the reveal is always telegraphed, and difficulty scales the
/// trigger range + shiver time like every other reaction window.
class BrambleMimicCore extends ThornlingCore {
  static const double triggerRange = 56; // px, scaled by mods.aggro
  static const double revealTime = 0.7; // s, scaled by mods.telegraph

  bool hidden = true;

  /// Telegraph seconds remaining once revealed; harmless until it hits 0.
  double revealLeft = 0;

  /// Render clock: seconds since the reveal started (0 while hidden).
  double sinceReveal = 0;

  bool _revealEdge = false;

  BrambleMimicCore({required super.x, required super.y})
    : super.asKind(EnemyKind.brambleMimic, hp: 8);

  @override
  bool get harmless => hidden || revealLeft > 0;

  /// One-shot: true exactly once after the reveal starts (session -> fx).
  bool takeJustRevealed() {
    final v = _revealEdge;
    _revealEdge = false;
    return v;
  }

  /// Back into the disguise (respawn-clear): a player walking back in gets
  /// the full telegraphed ambush again instead of a revealed hunter camping
  /// the checkpoint.
  void rehide() {
    if (!alive) return;
    hidden = true;
  ...
  void _reveal() {
    if (!hidden) return;
    hidden = false;
    revealLeft = revealTime * mods.telegraph;
    _revealEdge = true;
  }

  @override
  bool damage(int amount) {
    _reveal(); // poking a suspicious bush blows its cover
  ...
  void behave(
    ...
    if (hidden) {
      // A bush does not walk — but it does sit on the ground: settle under
      // gravity so a placement over uneven terrain can't leave it floating.
      body.vx = 0;
      body.vy += kGravity * dt;
      if (body.vy > kMaxFallSpeed) body.vy = kMaxFallSpeed;
      integrate(body, dt, tileAt);
      if ((playerX - centerX).abs() <= triggerRange * mods.aggro &&
          (playerY - centerY).abs() < 28) {
        _reveal();
      }
      return;
    }
    if (sinceReveal < 60) sinceReveal += dt;
    if (revealLeft > 0) {
      revealLeft -= dt;
      return; // shivering — not yet lunging
    }
    super.behave(dt, tileAt, playerX: playerX, playerY: playerY);
  }
}
...
class HopperCore extends EnemyCore {
  ...
  void behave(
    ...
    final playerVx = _lastPlayerX.isNaN ? 0.0 : (playerX - _lastPlayerX) / dt;
...
class SootCreeperCore extends EnemyCore {
  ...
  @override
  double get wakeDistance => kCreeperWakeDistance * mods.aggro;

  @override
  bool get hazardsKill => true;
