...
class ItemsComponent extends PositionComponent
  ...
  /// Max text width for the sign bubble: camera view minus bubble chrome
  /// (2px screen margin and 3px horizontal padding per side).
  ...
  void render(ui.Canvas canvas) {
    ...
    if (active != null && active.text.isNotEmpty) {
      if (!identical(active.text, _bubbleFor)) {
        ...
        // 352px view clipped regardless of the edge clamp below (the clamp
        // can only save bubbles NARROWER than the view). kBubbleMaxWidth
        // leaves a 2px margin + 3px padding per side.
        _bubbleTp = _bubbleText.toTextPainter(active.text)
          ..layout(maxWidth: kBubbleMaxWidth);
      }
      final tp = _bubbleTp!;
      final w = tp.width + 6, h = tp.height + 4;
      // Clamp the bubble inside the camera view: world-space centering used
      // to clip long sign text at the screen edge whenever the look-ahead
      // camera pushed the sign off-centre.
