...
/// Boss wake: the stone shell cracks off — a one-shot burst of grey shards
/// with a few moss flecks, tossed upward and pulled down by gravity, drawn as
/// tiny rects. Seeded, so store shots stay reproducible. All state is built
/// at construction; render is allocation-free.
class RubbleFx extends PositionComponent {
  static const _life = 0.7;
  static const _gravity = 340.0;
  final List<double> _vx, _vy, _size;
  final List<ui.Color> _colors;
  double _t = 0;
  final _paint = ui.Paint();

  /// [power] scales shard speed + size (boss death uses a heavier burst).
  RubbleFx(Vector2 at, {int seed = 7, int count = 14, double power = 1})
      : _vx = List.filled(count, 0),
        _vy = List.filled(count, 0),
        _size = List.filled(count, 0),
        _colors = List.filled(count, const ui.Color(0xFF8A8F85)),
        super(position: at, priority: 4) {
    final rng = math.Random(seed);
    for (var i = 0; i < count; i++) {
      _vx[i] = (rng.nextDouble() * 2 - 1) * 85 * power;
      _vy[i] = -(30 + rng.nextDouble() * 95) * power;
      _size[i] = (1.5 + rng.nextDouble() * 2.0) * power.clamp(1.0, 1.6);
      // Roughly one shard in four is a moss fleck, the rest stone.
      _colors[i] = rng.nextInt(4) == 0
          ? const ui.Color(0xFF6E8A5A)
          : const ui.Color(0xFF8A8F85);
    }
  }
  ...
  @override
  void render(ui.Canvas canvas) {
    final fade = _t < _life * 0.6
        ? 1.0
        : (1 - (_t - _life * 0.6) / (_life * 0.4)).clamp(0.0, 1.0);
    for (var i = 0; i < _vx.length; i++) {
      _paint.color = _colors[i].withValues(alpha: fade);
      final dx = _vx[i] * _t;
      final dy = _vy[i] * _t + 0.5 * _gravity * _t * _t;
      canvas.drawRect(
          ui.Rect.fromLTWH(dx, dy, _size[i], _size[i]), _paint);
    }
  }
}
...
class DeathFx extends SpriteAnimationComponent {
  DeathFx(Vector2 at, SpriteAnimation animation, {Vector2? size})
      : super(
          animation: animation,
          position: at,
          size: size ?? Vector2(40, 41),
