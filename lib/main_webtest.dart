...
void main() {
  ...
  // ?hearts=N clamps the player's hearts once (harness-only) so a death can
  // be staged from one hazard touch — death-beat capture (alpha.23).
  final heartsParam = int.tryParse(params['hearts'] ?? '') ?? -1;
  var heartsApplied = false;

  // ?controls=0.85|1.0|1.2 — Settings > Control size preview (alpha.23).
  final controls = double.tryParse(params['controls'] ?? '');
  // ?mirror=1 — Settings > Swap control sides preview (alpha.23).
  ...
  final slowmo = double.tryParse(params['slowmo'] ?? '');
  if (slowmo != null && slowmo > 0 && slowmo <= 1) game.timeScale = slowmo;
  ...
  Timer.periodic(const Duration(milliseconds: 50), (_) {
    ...
    try {
      ...
      if (!heartsApplied && heartsParam > 0 && game.sessionReady) {
        game.session.player.hearts = heartsParam.clamp(
          1,
          game.session.player.maxHearts,
        );
        heartsApplied = true;
      }
      ...
      home: Scaffold(
        ...
        body: Listener(
          ...
          child: GameWidget(
            ...
            overlayBuilderMap: {
              // ?unlocks=w1_boss previews the UNLOCKED rows (alpha.23) on a
              // clear of any level.
              EmberGame.overlayResults: (_, EmberGame g) => ResultsOverlay(
                results: g.session.results!,
                unlocks: unlocksOnFirstClear(params['unlocks'] ?? ''),
                onReplay: () {},
                onContinue: () {},
                onNext: () {}, // harness shows the 3-button layout
              ),
              EmberGame.overlayFail: (_, EmberGame g) =>
                  FailOverlay(onRetry: () {}, onLeave: () {}),
