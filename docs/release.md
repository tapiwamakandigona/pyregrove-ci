...
2. Bump `version:` in `pubspec.yaml` (semver-alpha + versionCode) AND `kAppVersion` in `lib/version.dart` (title-screen build label; test/version_test.dart fails the suite if they drift).
