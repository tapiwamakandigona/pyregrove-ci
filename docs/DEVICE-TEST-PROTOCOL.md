...
- **First PLAY of the session (alpha.23 #34–#36):** from the title screen,
  wait ~3 s (warm-ups run behind it), tap PLAY, and time PLAY → first
  complete frame with a stopwatch or screen recording (target: no visible
  black gap beyond the route transition; note the ms). The first frame must
  show tiles, player, HUD and backdrop together — no pop-in. Then jump once
  and swing once immediately: the first jump/swing sound must land on the
  action, not late. Repeat once after a force-stop (cold cache) and note
  both numbers; the cold one is the pillar-3 number.
