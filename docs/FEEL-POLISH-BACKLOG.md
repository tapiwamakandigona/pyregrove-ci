...
### B1. Tiered hitstop — DONE 2026-09-02 (tuning.dart kHitPause 0.035 / kKillPause 0.070 / kBossPhasePause 0.100 / kBossKillPause 0.22; session.dart picks by outcome; tests in combat_test, boss_core_test, kiln_golem_test — phase tests must pump through the freeze)
...
### B2. Per-weapon swing weight — DONE (feel-only variant) 2026-09-02 (lib/game/swing_weight.dart: weight derived from damage — starter light, hammer heavy, rest medium; heavy = hitstop ×1.35, arc stroke +1, camera thud 1.6 on plain connect. NO timing change: B6 rule kept for every weapon, so TTK/par_s untouched. The anticipation-ratio version stays open as a design call.)
...
### B3. Smear frames on the swing arc — DONE 2026-09-02 (player_component.dart `_smearPaint`: echo arc at swing−0.12 in the 0.25–0.85 window, stroke +1.5, 55 % alpha, white-shifted lead colour; verified visually via harness `?slowmo=`)
...
### B4. Kill permanence — DONE 2026-09-02 (CHECKED: enemies vanished with the DeathFx pop, nothing stayed. lib/game/components/ash_decal.dart: ash smudge at the feet of grounded non-boss kills, 3 flat ellipses, no asset, 10 s life / 2 s fade, cap 8 per level oldest-first; session event `ashLeft`; tests in ash_decal_test)
...
### B5. Escalating pickup pitch — DONE 2026-09-02 (audio_service.dart coinChainRate/coinChainAdvance: 1.5 s window, cap 8 → +8 semitones, composable with wobble; tests in audio_mix_test)
...
Status 2026-09-02: B1, B2 (feel-only), B3, B4, B5, B7 shipped on `main` (alpha.23); B6 rule preserved and tested. Backlog cleared; the timing-ratio version of B2 is a balance call for the owner.
