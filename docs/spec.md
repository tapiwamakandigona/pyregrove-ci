...
- Feel spec (all tunable constants in `lib/game/tuning.dart`):
  - **Coyote time** 0.12s; **jump buffer** 0.16s; variable jump height (release to cut). (Retuned from 0.10/0.12 per owner directive 2026-09-01 — rationale + sources in `docs/JUMP-PHYSICS.md`; `lib/game/tuning.dart` is canonical.)
...
## 5. Content plan
- **World 1 "Emberwood"** at alpha: 5 levels + boss arena; ≥2 secrets and 2–7 chests per level. (Shipped medal set is finish / all-chests / low-damage — no par times; this line's original "par times" idea was dropped in P-M8.)
- Level format: ASCII tile grids + entity legend in `assets/levels/*.txt` (headless-parseable, unit-tested, diff-friendly). World 2 shipped as **"Cinder Depths"** (5 levels + Kiln Golem boss, P-M9); World 3 ("The Delve") later.
...
## 8. Release targets (original plan — see status note)
...
> **Status (2026-09-02):** alpha.1–alpha.21 shipped as GitHub prereleases only.
> The "Play closed-track update" line is dice-era: that track belongs to the old
> package; Pyregrove goes up as a NEW Play listing if/when the owner says so
> (P-M10, owner call — see DEMAND.md; release freeze in effect). Quests (§4)
> remain post-alpha, not shipped.
