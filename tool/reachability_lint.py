...
Movement model (from lib/game/tuning.dart + test/physics_test.dart):
  ...
  * horizontal reach while airborne: ~6 columns (double jump at full run
    measures 6.33 tiles — pinned in test/jump_arc_test.dart; the content
    tests cap required gaps at maxGapTiles = 6)
  * falls: unlimited depth, same horizontal drift
  * cracked walls ('B') break from the side only (the swing hitbox is
    horizontal, player_core.dart attackHitbox): passable for horizontal
    entry and standable-on, but NOT risable-through — no upward swing can
    break a block overhead (model bug found 2026-09-02)
...
def lint(name):
    ...
    def pass_v(x, y):
        # Rising entry: no upward swing exists, an overhead 'B' is a ceiling.
        return (tile(x, y) not in (SOLID, "B")
                and tile(x, y - 1) not in (SOLID, "B"))
