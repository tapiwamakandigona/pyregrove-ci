...
void main() {
  test('totem range is eight tiles on medium, and the w2_l4 sign says so', () {
    // The sign used to claim "Totems spit farther in the dark" — nothing in
    // the game scales totem range by world or lighting (only difficulty
    // aggro does). Sign and constant are pinned together.
    expect(EmberTotemCore.range, 8 * kTileSize);
    final l =
        LevelData.parse(File('assets/levels/w2_l4.txt').readAsStringSync());
    expect(l.meta['sign1'], contains('eight tiles'));
  });
