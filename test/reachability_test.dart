...
const levels = [
  ...
  'w1_bonus', 'w2_bonus', // alpha.23 side content
];

List<String> loadGrid(String id) {
  final rows = File(
    'assets/levels/$id.txt',
  ).readAsLinesSync().where((l) => !l.startsWith('meta:')).toList();
  ...
  for (final id in levels) {
    test('$id: every collectible is reachable', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            '$id has unreachable collectibles: ${missed.join(', ')} '
