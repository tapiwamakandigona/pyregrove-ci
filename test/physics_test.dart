...
''');
  ...
  test('landedHard fires only for falls >= kHardLandTiles', () {
    // A normal jump on flat ground lands soft.
    final c = corePlayer(flat);
    var pressed = false;
    step(c, 1.0, (i) {
      if (!pressed) {
    ...
    final soft = c.takeEvents();
    expect(soft, contains(PlayerEvent.landed));
    expect(soft, isNot(contains(PlayerEvent.landedHard)));

    // Teleport-drop from kHardLandTiles + 1 tiles up: lands hard.
    final d = corePlayer(flat);
    d.body.y -= (kHardLandTiles + 1) * kTileSize;
    d.wasOnGround = false;
    step(d, 2.0, (_) {});
    final hard = d.takeEvents();
    expect(hard, contains(PlayerEvent.landed));
    expect(hard, contains(PlayerEvent.landedHard));
  });
