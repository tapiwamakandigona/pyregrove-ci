...
void main() {
  ...
  group('shipped levels', () {
    ...
    test('sign text uses the on-screen control vocabulary only', () {
      // Signs are the whole tutorial. The touch HUD labels its verbs JUMP,
      // SWORD, DASH (plus LEFT/RIGHT/DOWN on the pad), so a sign must never
      // name a chord or a keyboard key the player cannot see on screen —
      // w2_l1 shipped "Roll (DOWN+JUMP)" while w1_l1 taught "Tap DASH".
      final files = Directory('assets/levels')
          .listSync()
          .whereType<File>()
          .where((f) => f.path.endsWith('.txt'));
      final banned = RegExp(
        r'DOWN\s*\+\s*JUMP|\bShift\b|\bSpace\b|\bWASD\b|\bctrl\b|arrow key',
        caseSensitive: false,
      );
      // Nor may a sign order the player to do something the character
      // cannot: the verb set is run, jump, double-jump, swing, roll/dash,
      // drop-through, throw. "Duck under" shipped in two Ashbat signs while
      // no duck exists (and Ashbats weave in place — they never dive).
      final noSuchVerb = RegExp(
        r'(^|[.!?]\s+)(Duck|Crouch|Parry|Block|Slide|Glide|Swim|Fly)\b',
      );
      final signMeta = RegExp(r'^sign\d+$');
      var checked = 0;
      for (final f in files) {
        final l = LevelData.parse(f.readAsStringSync());
        for (final k in l.meta.keys.where(signMeta.hasMatch)) {
          checked++;
          expect(
            banned.hasMatch(l.meta[k]!),
            isFalse,
            reason: '${f.path} $k names a chord/key: "${l.meta[k]}"',
          );
          expect(
            noSuchVerb.hasMatch(l.meta[k]!),
            isFalse,
            reason: '${f.path} $k orders a verb the player lacks: '
                '"${l.meta[k]}"',
          );
        }
      }
