...
void main() {
  test('registry and files agree (every kWorld1 + bonus level exists)', () {
    for (final e in [...kWorld1, ...kBonusLevels]) {
      expect(
  ...
  // The World 1 bonus level (alpha.23) is held to every World 1 rule.
  for (final entry in [...kWorld1, ...kBonusLevels]) {
    group(entry.id, () {
      ...
      if (!entry.isBoss) {
        ...
        test('every secret chest sits behind a cracked wall or in a vault', () {
          ...
          for (final s in l.spawns.where(
            ...
            final near = walls.any(
              (w) => (w.$1 - s.x).abs() <= 5 && (w.$2 - s.y).abs() <= 3,
            );
            expect(
              near,
              isTrue,
      ...
      if (!entry.isBoss) {
        test('scripted runner bot reaches the door (completable)', () {
          expect(
            ...
            isTrue,
            reason:
                '${entry.id}: door-seeking runner bot never reached the exit',
          );
        });
      }
      ...
      test('spawns all required entities in a live session', () {
        ...
        switch (entry.id) {
          ...
          case 'w1_l2': // + hoppers (via meta) — thornlings/ashbats persist
            expect(
              s.enemies.whereType<HopperCore>().length,
              greaterThanOrEqualTo(2),
            );
            expect(s.enemies.whereType<ThornlingCore>(), isNotEmpty);
            expect(s.enemies.whereType<AshbatCore>(), isNotEmpty);
            expect(
              s.enemies.whereType<EmberTotemCore>(),
              isEmpty,
              reason: 'totems introduced in l3',
            );
          case 'w1_l3': // + ember totems
            expect(
              s.enemies.whereType<EmberTotemCore>().length,
              greaterThanOrEqualTo(2),
            );
            expect(
              s.enemies.whereType<RotshieldCore>(),
              isEmpty,
              reason: 'rotshields introduced in l4',
            );
          case 'w1_l4': // + rotshields
            expect(
              s.enemies.whereType<RotshieldCore>().length,
              greaterThanOrEqualTo(2),
            );
          ...
          case 'w1_bonus': // side content: the full World 1 roster, no boss
            expect(
              s.enemies.whereType<EmberTotemCore>().length,
              greaterThanOrEqualTo(2),
            );
