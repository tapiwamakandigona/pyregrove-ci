...
void main() {
  ...
  test('phase thresholds: 2/3 -> phase 2, 1/3 -> phase 3, with events', () {
    ...
    pump();
    expect(boss.phase, 2);
    // B1 tiered hitstop: a phase threshold is a chapter break — it holds
    // the frame for kBossPhasePause (longer than any ordinary connect).
    expect(s.hitPause, closeTo(kBossPhasePause, 1e-9));
    boss.hp = (GroveGolemCore.maxHp / 3).floor(); // 50 -> phase 3
    // The phase-2 hitstop freezes the sim for kBossPhasePause; pump
    // through it so the phase-3 scan can run.
