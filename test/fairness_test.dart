...
//   4. a casual bot's run, if it dies out, must still last >=12 s and reach
//      >=30% of the level (bosses exempt: a bot that never aims should lose)
import 'dart:io';
...
/// Everything that is NOT an enemy. The enemy set is derived by exclusion so
/// a newly added SpawnKind is treated as dangerous by default — a
/// hand-enumerated enemy list silently exempted new enemies from the runway
/// rule (the Bramble Mimic was missing from it until 2026-09-02).
const _nonEnemyKinds = {
  SpawnKind.player,
  SpawnKind.exit,
  SpawnKind.sign,
  SpawnKind.checkpoint,
  SpawnKind.coin,
  SpawnKind.apple,
  SpawnKind.feather,
  SpawnKind.heart,
  SpawnKind.chest,
  SpawnKind.secretChest,
};
final _enemyKinds = SpawnKind.values.toSet().difference(_nonEnemyKinds);
...
void main() {
  for (final id in levels) {
    group(id, () {
      test('the spawn runway is safe', () {
        ...
        for (final s in l.spawns) {
          if (_enemyKinds.contains(s.kind) && s.x > spawn.x && s.x <= limit) {
            fail(
              '$id: ${s.kind} at x=${s.x} is inside the spawn runway '
      ...
      test('has campfire checkpoints', () {
        final l = load(id);
        final fires = l.spawns
            .where((s) => s.kind == SpawnKind.checkpoint)
            .toList();
        expect(
          fires.length,
          greaterThanOrEqualTo(id.endsWith('boss') ? 1 : 2),
        ...
        if (fires.length >= 2) {
          ...
          expect(
            fires.last.x - fires.first.x,
            greaterThan(l.width ~/ 5),
      ...
      test('a casual player is not wiped out in the opening seconds', () {
        ...
        if (s.failed) {
          expect(
            ...
            reason:
                '$id: run wiped out after ${t.toStringAsFixed(1)}s '
  ...
  group('checkpoints and lives', () {
    test('a death spends a life and rewinds to the lit campfire', () {
      ...
      final deathX = s.player.body.centerX;
      s.player.kill();
      s.update(dt, input..dirX = 0);
      expect(s.failed, isFalse, reason: 'a life must absorb the death');
      expect(s.lives, livesBefore - 1);
      expect(s.deaths, 1);
      // Death beat (alpha.23): the sim holds kDeathHold at the death spot —
      // player still dead, not yet moved — and reports where it happened.
      final died = s.takeEvents().where(
        (e) => e.kind == SessionEventKind.playerDied,
      );
      expect(died.length, 1);
      expect(died.first.x, closeTo(deathX, 1e-6));
      expect(s.player.isDead, isTrue, reason: 'no instant revive');
      expect(s.player.body.centerX, closeTo(deathX, 1e-6));
      final timeBefore = s.time;
      var held = 0.0;
      while (s.player.isDead && held < kDeathHold + 0.2) {
        s.update(dt, input);
        held += dt;
      }
      expect(
        held,
        closeTo(kDeathHold, dt * 3),
        reason: 'respawn lands right after the hold, not before',
      );
      expect(
        s.time,
        closeTo(timeBefore, dt * 1.5),
        reason: 'the hold does not run the clock (only the respawn frame)',
      );
      expect(
        s.takeEvents().any((e) => e.kind == SessionEventKind.respawned),
        isTrue,
      );
      expect(s.player.hearts, s.player.maxHearts, reason: 'respawn heals');
      expect(
        s.player.body.centerX,
        closeTo(fire.x, 2),
    ...
    test('running out of lives still ends the run', () {
      ...
      for (var i = 0; i < kStartingLives; i++) {
        s.player.kill();
        // Ride out the death hold before the next death.
        for (var t = 0.0; t < kDeathHold + 0.1; t += dt) {
          s.update(dt, input);
        }
      }
      expect(s.lives, 0);
    ...
    test('the last life also gets the death beat before FALLEN', () {
      final s = LevelSession(load('w1_l1'), Loadout.starter(), seed: 3);
      final input = InputIntent();
      s.lives = 1;
      s.player.kill();
      s.update(dt, input);
      expect(s.lives, 0);
      expect(s.failed, isFalse, reason: 'held at the death spot first');
      expect(
        s.takeEvents().any((e) => e.kind == SessionEventKind.playerDied),
        isTrue,
      );
      var held = 0.0;
      while (!s.failed && held < kDeathHold + 0.2) {
        s.update(dt, input);
        held += dt;
      }
      expect(s.failed, isTrue);
      expect(held, closeTo(kDeathHold, dt * 3));
    });
