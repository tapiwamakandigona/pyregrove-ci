            ...
            if (boss.isEmpty) {
              ...
              for (final h in b.hazards) {
                ...
                if (h.kind == BossHazardKind.emberBomb && h.vy > -60) {
                  // Falling ember: project its landing x, step out from under
                  // it. 2026-09-01: widened 0.35s/30px -> 0.5s/38px — from
                  // the retuned 412 perch anchor the lob arrives on a
                  // steeper leg and the old window flagged it too late
                  // (seed-99 wipe: bombs at 8-15 px on the perch).
                  final landX = h.x + h.vx * 0.5;
                  if ((landX - px).abs() < 38) sidestep = landX > px ? -1 : 1;
                }
              }
              ...
              if (caged) {
                const perchX = 418.0; // 2026-09-01: 424 - 6, splits contact-arrival vs bomb-dodge geometry after arc retune
                final onPerch =
                    s.player.body.bottom <= 209 && px > 384 && px < 434;
                if (!onPerch) {
                  // Navigate: run right, jump the fire moat, double-jump to top.
                  // Airborne, brake 14 px early: air momentum carries ~15-20
                  // px past the steer flip with the 2026-09-01 floatier arc,
                  // which used to end in the boss's contact envelope.
                  final navTarget =
                      s.player.body.onGround ? perchX : perchX - 14;
                  input.dirX = px < navTarget ? 1 : -1;
                  ...
                  // 2026-09-01 arc retune (wider apex hang): release the
                  // hold over the pillar so the arc drops onto the perch
                  // instead of hanging past it into the boss pen — the old
                  // arc landed here by itself; the floatier one needs the
                  // deliberate release a human would also make.
                  if (px > 398 && s.player.body.vy < 0) {
