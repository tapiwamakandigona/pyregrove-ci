...
void main() {
  ...
  test('B1 tiered hitstop: plain connect kHitPause, killing blow kKillPause',
      () {
    // Plain connect: squire blade (3 dmg) vs 6 hp thornling — survives.
    // Pick a seed whose first combat roll is NOT a crit (a crit would kill).
    var seed = 0;
    while (Rng.create(seed, 'combat').range(1, 100) <=
        weaponById('squire_blade').critPercent) {
      seed++;
    }
    final s = combatSession(seed: seed);
    final intent = InputIntent()..attackPressed = true;
    ...
    expect(s.enemies.single.alive, isTrue);
    expect(s.hitPause, closeTo(kHitPause, dt),
        reason: 'a surviving enemy holds only the plain-connect pause');

    // Killing blow: the hammer (9 dmg) one-shots the 6 hp thornling.
    final sk = combatSession(weapon: weaponById('wind_gods_hammer'));
    final ik = InputIntent()..attackPressed = true;
    frames = 0;
    while (sk.hitPause <= 0 && frames < 60) {
      sk.update(dt, ik);
      ik.clearEdges();
      frames++;
    }
    expect(sk.enemies.single.alive, isFalse);
    // B2 swing weight: the heavy hammer multiplies the pause (feel-only).
    expect(sk.hitPause, closeTo(kKillPause * hitPauseMul(SwingWeight.heavy), dt),
        reason: 'a killing blow holds the frame twice as long, x heavy');
    expect(hitPauseMul(swingWeightFor(weaponById('wind_gods_hammer'))),
        greaterThan(1.0));
  });
